...
void ClientNetworkComInit(char ip[],u_short port)
{
	...
	do
	{
		cstate = send(ServerCom, password, 512, 0);
	...
	} while (strcmp(cMessage, "Enter a valid password") );
	cin >> cMsg;
	strcpy_s(cMessage, id);
	strcat_s(cMessage, cMsg);
	cout << cMessage;
	cin.ignore();
